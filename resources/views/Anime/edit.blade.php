...
<div class="container">
    ...
    @endif
    <h1>Data Anime</h1>
    <form action="{{route('dataAnime.update',['id_anime' => @$anime->id_anime])}}" method="POST">
        {{--{{ url('dataAnime', ['id_anime' => @$anime->id_anime]).'/update' }}--}}
        @csrf
        @method('PATCH')
        ...
        <div class="form-group">
            ...
            <div>
                @if(@$anime->image_url)
                    <img src="{{ url('uploads/'.@$anime->image_url) }}" class="img-thumbnail" width="150px"
                         alt="image"/>
                @endif
                <input type="file" class="form-control-file mt-1" name="image_url" readonly disabled>
            </div>
        </div>
        <div class="form-group">
